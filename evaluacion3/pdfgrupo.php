...
$pdf=new PDF;
$link=Conectarse();
$consultatodosalumnos = mysql_query("SELECT * from alumnos ORDER BY apellidos",$link);


while ($row = mysql_fetch_array($consultatodosalumnos)) {
$consultagrupoalumno = mysql_query("SELECT alumno FROM matriculas WHERE grupo='$grupo' AND alumno='$row[alumno]'",$link);
while ($res=mysql_fetch_array($consultagrupoalumno)){

$nene3=mysql_query("SELECT * FROM alumnos WHERE alumno='$res[alumno]'",$link);

while ($nene2=mysql_fetch_array($nene3)){
